...
- `LAST30DAYS_LIBRARY_DB=<path>` - override the shared SQLite library index path (default: `~/.local/share/last30days/library.db`). Use this when the default user-profile location is unavailable to your agent sandbox; an explicit path is used as-is (after `~` expansion) and errors are reported without fallback.
